...
type ChartTooltipContentProps = {
  ...
  formatter?: (value: number, name: string, item: TooltipPayload, index: number, payload: Record<string, any>) => React.ReactNode;
  color?: string;
  nameKey?: string;
  labelKey?: string;
};
            ...
            return (
              ...
              >
                {formatter && item?.value !== undefined && item.name ? (
                  formatter(item.value || 0, item.name || '', item, index, item.payload || {})
                ) : (
                  <>
                    {itemConfig?.icon ? (
                      <itemConfig.icon />
